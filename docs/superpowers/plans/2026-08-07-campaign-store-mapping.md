...
describe('syncAdAccount for a split account', () => {
  beforeEach(() => {
    vi.mock('./google', async (importOriginal) => ({
      ...
      fetchGoogleCampaignDaily: vi.fn(async () => [
        {
          campaignId: 'c1', campaignName: 'Norway', date: new Date('2026-03-01T00:00:00Z'),
          spend: 1000, impressions: 5, clicks: 1, linkClicks: 1,
          conversions: 0, conversionValue: 0, videoViews3s: 0, thruplays: 0, reach: 0,
        },
        {
          campaignId: 'c2', campaignName: 'Sweden', date: new Date('2026-03-01T00:00:00Z'),
          spend: 2000, impressions: 7, clicks: 2, linkClicks: 2,
          conversions: 0, conversionValue: 0, videoViews3s: 0, thruplays: 0, reach: 0,
        },
      ]),
  ...
  it('writes one AdCampaign and one AdCampaignSpend per campaign', async () => {
    ...
    vi.spyOn(await import('./sync'), 'resolveCredentials').mockResolvedValue({
    ...
    const campaigns = await db.adCampaign.findMany({ where: { accountId: account.id }, orderBy: { externalId: 'asc' } })
    expect(campaigns.map((c) => c.externalId)).toEqual(['c1', 'c2'])
    expect(campaigns.every((c) => c.shopId === null)).toBe(true)

    const spend = await db.adCampaignSpend.findMany({ where: { campaignId: { in: campaigns.map((c) => c.id) } } })
  ...
  it('refreshes a renamed campaign without unassigning its store', async () => {
    ...
    expect(c1?.name).toBe('Norway')   // refreshed
    expect(c1?.shopId).toBe(shop.id)  // survived
  })
})
```
