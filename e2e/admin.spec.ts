...
async function signIn(page: import('@playwright/test').Page, email: string) {
  ...
  // The click only dispatches the DOM event â€” it does not wait for the async
...
test('admin sees the dashboard with real figures', async ({ page }) => {
  ...
  // Profit is the headline figure â€” the question the page exists to answer.
...
test('the dashboard charts revenue and profit over time', async ({ page }) => {
  ...
  // Two series, so both must be named â€” identity never rests on colour alone.
...
test('every headline figure says which way it moved', async ({ page }) => {
  ...
  await expect(page.getByText(/[+âˆ’]\d+\.\d%/).first()).toBeVisible()
})
...
test('isolating a single shop switches to that shop own currency', async ({ page }) => {
  ...
  await expect(page.getByRole('row', { name: /Total/ })).toContainText('USD')
  ...
  // One NOK shop -> figures are in kroner (Nordic style shows "kr"), not USD.
  await expect(page.getByRole('row', { name: /Total/ })).toContainText('kr', { timeout: 10_000 })
  await expect(page.getByRole('row', { name: /Total/ })).not.toContainText('USD')
})
