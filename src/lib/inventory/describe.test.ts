...
describe('describeSources', () => {
  it('names the shops the stock came from, and says sales came from all of them', () => {
    expect(describeSources(['Panetti Norway', 'Mazzetti Norway'], 9)).toBe(
      'Stock from Panetti Norway and Mazzetti Norway. Sales counted from all 9 shops.',
    )
  })

  it('reads naturally for a single source', () => {
    expect(describeSources(['Panetti Norway'], 9)).toBe(
      'Stock from Panetti Norway. Sales counted from all 9 shops.',
    )
  })

  it('uses commas and a final "and" for three or more', () => {
    expect(describeSources(['A', 'B', 'C'], 9)).toBe(
      'Stock from A, B and C. Sales counted from all 9 shops.',
    )
  })
  ...
  /**
   * Ticking every shop is the same set as ticking none, so it must not produce a
   * sentence listing nine names — which would be both unreadable and a
