...
/**
 * The Add an ambassador tab: the form, and under it the roster - every
 * ambassador with their commission, codes, products, status and invite link.
 *
 * It is the chore side of Ambassadors. The statistics on the first tab
 * (AmbassadorsClient) are read; this tab is where things are done, so it
 * carries none of the shop and period filters that drive those figures.
 */
export function AddAmbassadorClient({
  email,
  role = 'ADMIN',
}: {
  email: string
  ...
  // The form's tick list and the modal's picker both read the catalogue. The
  // same request also carries the product overview, which is the other tab's.
  ...
  /** Refreshed alongside the roster: a product handed out can change what a store offers. */
  const loadProducts = useCallback(async () => {
    try {
      const res = await fetch('/api/ambassador-products')
      if (!res.ok) return // the picker simply stays as it was
      const data = (await res.json()) as { catalogue?: CatalogueItem[] }
  ...
  useEffect(() => {
    ...
    fetch('/api/ambassador-products')
      .then(async (r) => (r.ok ? ((await r.json()) as { catalogue?: CatalogueItem[] }) : null))
      .then((data) => {
        if (live && data) setCatalogue(data.catalogue ?? [])
      })
      .catch(() => {})
    return () => {
      live = false
  ...
  const send: Send = async (key, url, method, body) => {
    ...
    try {
      ...
      // Both, because adding a product changes the roster chips AND the catalogue.
