...
function renderPage(ambassadors: unknown[] = [], role: 'ADMIN' | 'MARKETING' = 'ADMIN') {
  ...
  render(
    <ToastProvider>
      <AddAmbassadorClient email="admin@test.local" role={role} />
    </ToastProvider>,
  )
}

/** The form's own heading, not the tab of the same name above it. */
const formHeading = () => screen.getByRole('heading', { name: 'Add an ambassador' })

describe('the Add an ambassador tab', () => {
  it('holds the form and the roster, under the tabs, with no statistics or their filters', async () => {
    renderPage([
      {
    ...
    expect(screen.getByTestId('add-ambassador')).toBeTruthy()

    const tabs = within(screen.getByRole('navigation', { name: 'Section' }))
    expect(tabs.getByRole('link', { name: 'Add an ambassador' }).getAttribute('aria-current')).toBe('page')
    expect(tabs.getByRole('link', { name: 'Ambassadors' }).getAttribute('href')).toBe('/ambassadors')

    // The figures and the filters that drive them belong to the other tab.
    expect(screen.queryByText('Top ambassadors')).toBeNull()
    expect(screen.queryByLabelText('Shops')).toBeNull()
    expect(screen.queryByLabelText('Period')).toBeNull()
    expect(screen.queryByText('Products with ambassadors')).toBeNull()
  })

  it('never asks for the statistics it does not show', async () => {
    renderPage()
    await waitFor(() => expect(formHeading()).toBeTruthy())
    const calls = (globalThis.fetch as unknown as { mock: { calls: [string][] } }).mock.calls
    expect(calls.some(([url]) => String(url).includes('/api/ambassadors/stats'))).toBe(false)
  })
...
describe('AddAmbassadorClient store-scoped codes', () => {
